...
struct MPCData
{
  ...
  double slip_angle{};
  double lateral_err_cg{};
  double yaw_err_cg{};
  size_t nearest_idx_cg;
  double nearest_time_cg;
  geometry_msgs::msg::Pose nearest_pose_cg;
...
private:
  ...
  double m_steer_prev = 0.0;           // Previous Steering Tire Angle.
  ...
  geometry_msgs::msg::Pose m_prev_pose;
  ...
  /// 最新のリサンプリング済み参照軌跡
  MPCTrajectory mpc_resampled_ref_traj_;
...
public:
  ...
  /** @brief リサンプリング済み参照軌跡（mpc_resampled_ref_trajectory）を取得 */
  const MPCTrajectory & getResampledReferenceTrajectory() const
  {
    return mpc_resampled_ref_traj_;
  }
};  // class MPC
}  // namespace autoware::motion::control::mpc_lateral_controller
