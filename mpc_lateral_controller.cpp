...
#include "mpc_lateral_controller/vehicle_model/itc_vehicle_kinematics.hpp"
#include "tf2/utils.h"
...
{
  // ── 新パラメータ ──
  enable_ref_traj_pub_ = node.declare_parameter<bool>("publish_reference_trajectory", true);
  const std::string ref_traj_topic = 
    node.declare_parameter<std::string>("reference_trajectory_topic", "mpc/reference_trajectory");


  if (enable_ref_traj_pub_) {
    reference_traj_pub_ = node.create_publisher<Trajectory>(
      ref_traj_topic,
      rclcpp::QoS{1}.transient_local());  // “最新1件をラッチ” しておくとデバッグに便利
  }
  ...
  const double wheelbase, const double steer_lim, const double steer_tau, rclcpp::Node & node)
{
  ...
  if (vehicle_model_type == "itc"){
    const double c_f = node.declare_parameter<double>("c_f");					
    const double c_r = node.declare_parameter<double>("c_r");					
    const double i_f = node.declare_parameter<double>("i_f");					
    const double i_r = node.declare_parameter<double>("i_r");					
    const double i_z = node.declare_parameter<double>("i_z");					
    const double mass_v = node.declare_parameter<double>("mass_v");					
    const double lag = node.declare_parameter<double>("lag");	
    const double mv_sf = node.declare_parameter<double>("mv_sf");
    const double md_sf = node.declare_parameter<double>("md_sf");				
    vehicle_model_ptr = std::make_shared<ITCKinematicsBicycleModel>(wheelbase, 
                                                                    steer_lim, 
                                                                    steer_tau,
                                                                    c_f, c_r, 
                                                                    i_f, i_r, 
  ...
  trajectory_follower::InputData const & input_data)
{
  ...
  if (enable_ref_traj_pub_ && reference_traj_pub_
      && reference_traj_pub_->get_subscription_count() > 0) {
        const auto & resampled = m_mpc.getResampledReferenceTrajectory();
        auto msg = MPCUtils::convertToAutowareTrajectory(resampled);
    msg.header.stamp    = clock_->now();
    msg.header.frame_id = "map";
    reference_traj_pub_->publish(msg);
  }
