...
#include "mpc_lateral_controller/vehicle_model/itc_vehicle_kinematics.hpp" // ITCKinematicsBicycleModel をインクルード
  ...
  Float32MultiArrayStamped & diagnostic)
{
  ...
  const auto [success_resample, mpc_resampled_ref_trajectory] =
    ...
    mpc_resampled_ref_traj_ = mpc_resampled_ref_trajectory;  // ← 追加
  if (!success_resample) {
  ...
  const VectorXd & x0_delayed, const VectorXd & Uex) const
{
  ...
  // predicted_traj.header.frame_id = "map";               // 必要なら適宜
  // predicted_traj.header.stamp    = clock_->now();       // 〃
  
  const auto & traj = mpc_resampled_ref_trajectory;

  const double lr = 1.39; // m_vehicle_model_ptr->getL();  // ensure lr is available // added by Patel 2025-07-23
  ...
  for (int i = 0; i < m_param.prediction_horizon; ++i) {
    ...
    const double yaw_error_rear = yaw_error - std::atan(lr * traj.k.at(i));	// added by Patel 2025-07-23
    const double lat_error_rear = lat_error - lr * std::sin(yaw_error_rear);	
    
    //const double x = traj.x.at(i) - std::sin(traj.yaw.at(i)) * lat_error;
    //const double y = traj.y.at(i) + std::cos(traj.yaw.at(i)) * lat_error;    
    const double x = traj.x.at(i) - std::sin(traj.yaw.at(i)) * lat_error_rear;	// added by Patel 2025-07-23
    const double y = traj.y.at(i) + std::cos(traj.yaw.at(i)) * lat_error_rear;	// added by Patel 2025-07-23
    const double z = traj.z.at(i);
    //const double yaw = traj.yaw.at(i) + yaw_error;
    const double yaw = traj.yaw.at(i) + yaw_error_rear;		// added by Patel 2025-07-23
    ...
    /* ---------------- 追加したい MPC 状態 ---------------- */
    const double front_wheel_angle = Xex(i * DIM_X + 2);
    const double lateral_vel       = Xex(i * DIM_X + 3);   // itc のときだけ使う
    const double yaw_rate          = Xex(i * DIM_X + 4);   // itc のときだけ使う
    ...
    p.pose.position.y          = y;
    p.pose.position.z          = z;
    p.pose.orientation         = tier4_autoware_utils::createQuaternionFromYaw(yaw);
    p.longitudinal_velocity_mps = static_cast<float>(vx);

    // ★ ここで要求値を格納
    p.front_wheel_angle_rad    = static_cast<float>(front_wheel_angle);

    if (std::dynamic_pointer_cast<ITCKinematicsBicycleModel>(m_vehicle_model_ptr)) {
      p.lateral_velocity_mps   = static_cast<float>(lateral_vel);
      p.heading_rate_rps       = static_cast<float>(yaw_rate);
    }
  ...
  const Odometry & current_kinematics) const
{
  ...
  append_diag(mpc_data.lateral_vel);           // [21] lateral velocity (Vy)
  append_diag(mpc_data.slip_angle);           // [22]
  append_diag(mpc_data.lateral_err_cg);           // [23]
  append_diag(mpc_data.yaw_err_cg);           // [24]
  ...
  const Odometry & current_kinematics)
{
  ...
  // add lateral_vel and yaw_rate
  //data.lateral_vel = current_kinematics.twist.twist.linear.y;
  double lr = 1.53;
  
  // スマートポインタの場合
  auto itc_model = std::dynamic_pointer_cast<ITCKinematicsBicycleModel>(m_vehicle_model_ptr);
  if (itc_model) {
    lr = itc_model->get_ir();
  } else {
    RCLCPP_WARN(m_logger, "Vehicle model is not ITCKinematicsBicycleModel. Using default lr = 1.53");
  }

  // ハードコードされた lr を置き換え
  
  const double dt = m_ctrl_period;
  const double dx = current_pose.position.x - m_prev_pose.position.x;
  const double dy = current_pose.position.y - m_prev_pose.position.y;
  const double yaw = tf2::getYaw(current_pose.orientation) ;
  const double prev_yaw = tf2::getYaw(m_prev_pose.orientation) ;
  const double avg_yaw = (yaw + prev_yaw) / 2.0 ;

  geometry_msgs::msg::Pose cg_pose = current_pose;
  cg_pose.position.x += lr * std::cos(yaw);
  cg_pose.position.y += lr * std::sin(yaw);

  if (!MPCUtils::calcNearestPoseInterp(
    traj, cg_pose, &(data.nearest_pose_cg), &(data.nearest_idx_cg), &(data.nearest_time_cg), ego_nearest_dist_threshold, ego_nearest_yaw_threshold)){
      warn_throttle("calculateMPC: error in calculating nearest pose for CG. stop mpc.");
      return {false, MPCData{}};
    }

  data.lateral_err_cg = MPCUtils::calcLateralError(cg_pose, data.nearest_pose_cg);
  
  const double yaw_err_kinematics_cg = normalizeRadian(tf2::getYaw(cg_pose.orientation) - tf2::getYaw(data.nearest_pose_cg.orientation));
  
  if (std::abs(current_kinematics.twist.twist.linear.x) < 1.0){
    data.lateral_vel = 0.0;
    data.slip_angle = 0;
  } else {
    data.lateral_vel = (- dx * std::sin(avg_yaw) + dy * std::cos(avg_yaw))/dt + lr * current_kinematics.twist.twist.angular.z;
    data.slip_angle = data.lateral_vel / current_kinematics.twist.twist.linear.x;
  }
  
  data.yaw_err_cg = yaw_err_kinematics_cg + data.slip_angle;  
  m_prev_pose = current_pose;
  data.yaw_rate = current_kinematics.twist.twist.angular.z;
...
VectorXd MPC::getInitialState(const MPCData & data)
{
  ...
  const auto & lat_err_cg = data.lateral_err_cg;
  const auto & yaw_err_cg = data.yaw_err_cg;
  // add lateral_vel and yaw_rate
  const auto & lateral_vel = data.lateral_vel;
  const auto & yaw_rate = data.yaw_rate;

  const auto vehicle_model = m_vehicle_model_ptr->modelName();
  // add else if for itc model
  if (vehicle_model == "kinematics") {
  ...
  } else if (vehicle_model == "itc") {
    // change lateral_vel -> dlat, yaw_rate -> dyaw
    //double dlat = (lat_err - m_lateral_error_prev) / m_ctrl_period;
    //double dyaw = (yaw_err - m_yaw_error_prev) / m_ctrl_period;
    //m_lateral_error_prev = lat_err;
    //m_yaw_error_prev = yaw_err;
    //dlat = m_lpf_lateral_error.filter(dlat);
    //dyaw = m_lpf_yaw_error.filter(dyaw);
    //x0 << dlat, dyaw, lat_err, yaw_err;
    //x0 << lat_err, yaw_err, m_steer_prev, lateral_vel, yaw_rate;
    //m_steer_prev = steer;
    x0 << lat_err_cg, yaw_err_cg, steer, lateral_vel, yaw_rate;
    //double steer2 = steer * 180.0 / 3.1415926;
    //RCLCPP_INFO(m_logger, "steer data = %f[rad], %f[deg]", steer, steer2);
  } else {
  ...
  const MPCTrajectory & traj, const double & start_time, const VectorXd & x0_orig)
{
  ...
  for (size_t i = 0; i < m_input_buffer.size(); ++i) {
    ...
    // add print debug
    //RCLCPP_DEBUG(m_logger, "[itc_vehicle] a_d = [%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f]", Ad(0,0), Ad(0,1), Ad(0,2), Ad(0,3), Ad(1,0), Ad(1,1), Ad(1,2), Ad(1,3), Ad(2,0), Ad(2,1), Ad(2,2), Ad(2,3), Ad(3,0), Ad(3,1), Ad(3,2), Ad(3,3));
    //RCLCPP_DEBUG(m_logger, "[itc_vehicle] b_d = [%f, %f, %f, %f]", Bd(0,0), Bd(1,0), Bd(2,0), Bd(3,0));
    //RCLCPP_DEBUG(m_logger, "[itc_vehicle] c_d = [%f, %f, %f, %f, %f, %f, %f, %f]", Cd(0,0), Cd(0,1), Cd(0,2), Cd(0,3), Cd(1,0), Cd(1,1), Cd(1,2), Cd(1,3));
    //RCLCPP_DEBUG(m_logger, "[itc_vehicle] w_d = [%f, %f, %f, %f]", Wd(0,0), Wd(1,0), Wd(2,0), Wd(3,0));				
  ...
  const MPCTrajectory & reference_trajectory, const double prediction_dt)
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    // new conversion to CoG curvature: // added 2025-07-18 by Patel
    double lr = 1.39; // m_vehicle_model_ptr->getLr();  // ensure lr is available
    double ref_k_cg = ref_k / (1.0 - lr * ref_k);
    double ref_smooth_k_cg = ref_smooth_k / (1.0 - lr * ref_smooth_k);

    // get discrete state matrix A, B, C, W
    m_vehicle_model_ptr->setVelocity(ref_vx);
    //m_vehicle_model_ptr->setCurvature(ref_k);
    m_vehicle_model_ptr->setCurvature(ref_k_cg); // added 2025-07-18 by Patel
    ...
    m_vehicle_model_ptr->setCurvature(ref_smooth_k_cg); // added 2025-07-18 by Patel
  ...
  addSteerWeightR(prediction_dt, m.R2ex);
